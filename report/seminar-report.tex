...
\lstset{
  ...
  frame=single,         % Optional: adds a box around the prompt
...
% TODO add reference to model parameters (table in appendix)
...
This formatting ensures that the model switches to the appropriate response mode (letter-selection for \acp{mcq} vs. concise naming for \acp{saq}). Examples of the final training prompt structures for both \ac{mcq} and \ac{saq} tasks are provided in \textbf{Appendix \ref{sec:appendix_prompts}}.
With this change a training-prompt for \ac{mcq} looks like this:
...
% TODO add graph of improvement steps from notebook
